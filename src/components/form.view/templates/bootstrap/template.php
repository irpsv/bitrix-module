...
$htmlId = "formViewBootstrap".uniqid();
...
?>
<div id="<?= $htmlId ?>" class="formViewBootstrap" data-ajax='<?= $isAjax ? 1 : 0 ?>' data-success='<?= $successText ?>'>
	<?php
	if ($isAjax) {
		echo "<form action='{$action}' method='{$method}' name='{$formName}' onsubmit='return false;'>";
	}
	else {
	...
	?>
		...
		foreach ($fields as $field) {
			...
			echo "<div class='formViewBootstrap__item form-group {$required}'>";
			if ($type === 'textarea') {
		...
		?>
		<div class="formViewBootstrap__buttons form-group">
			<button type="submit" class="btn btn-primary">
				<?= $buttonText ?>
			</button>
		</div>
		<div class="formViewBootstrap__alert alert">
...
<script type="text/javascript">
	window.formViewBootstrapLabels = window.formViewBootstrapLabels || {};
	window.formViewBootstrapLabels['<?= $htmlId ?>'] = <?= json_encode($labels) ?>;
</script>
